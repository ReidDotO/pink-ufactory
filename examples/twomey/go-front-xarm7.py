...
if not params['quit']:
    ...
    if arm.error_code == 0 and not params['quit']:
        # Forward position for xarm7 - extends the arm forward
        ...
        # Forward position: joint2 and joint3 bent to extend arm forward, joint5 and joint6 adjusted for orientation
        code = arm.set_servo_angle(angle=[0, 2.5, 0, 37.3, 0, -57.3, 179.0], speed=params['angle_speed'], mvacc=params['angle_acc'], wait=True, radius=-1.0)
        # Alternative forward position with different joint angles
        # code = arm.set_servo_angle(angle=[0.0, 56.2, 74, 0.0, -84.7, -90.0, 0.0], speed=params['angle_speed'], mvacc=params['angle_acc'], wait=True, radius=-1.0)
        if code != 0:
            params['quit'] = True
